...
async def _request(req: Request):
...
async def _follow_redirects(resp: Response, max_redirects: int) -> Response:
    history = []
...
async def request(
    ...
    *,
    ...
    response = await _request(req)
    if follow_redirects:
        response = await _follow_redirects(response, max_redirects)
    if raise_for_status:
